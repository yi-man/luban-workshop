...
def login(phone: str, headless: bool):
    ...
    async def do_login():
        ...
        console.print(f"[blue]正在启动浏览器...[/blue]")
        ...
        async with async_playwright() as p:
            ...
            with Status("[blue]正在加载登录页面...", console=console) as status:
                # 访问登录页面
                await page.goto("https://bigmodel.cn/glm-coding")
                await page.click("text=登录 / 注册")
                status.update("[blue]等待页面加载...")
                await asyncio.sleep(1)

            # 输入手机号
            console.print(f"[blue]正在输入手机号...[/blue]")
            ...
            try:
                ...
                if user_element:
                    ...
                    console.print("[yellow]⚠ 无法确认登录状态，但会继续保存cookies[/yellow]")
            except:
                pass
...
def check_login():
    ...
    try:
        ...
        # 统计cookie信息
        total = len(cookies)
        expired = [c for c in cookies if c.get("expires", float("inf")) < time.time()]
...
async def _buy(package: str, period: str, target_time: str, headless: bool, now: bool = False):
    """抢购实现"""
    from datetime import datetime, timedelta
    ...
    try:
        with open(cookies_file) as f:
            cookies = json.load(f)
        valid_cookies = [c for c in cookies if c.get("expires", float("inf")) > time.time()]
    ...
    if not product_id:
        ...
        console.print("[dim]请检查套餐类型和订阅周期是否正确[/dim]")
        return
    ...
    if not now:
        ...
        if start_time > now:
            ...
            console.print(f"[blue]等待到 {start_time.strftime('%H:%M:%S')} (还有 {time_str})[/blue]")
...
async def _test(headless: bool):
    ...
    with Status("[blue]测试产品ID查询...", console=console) as status:
        try:
            ...
            all_found = True
            for pkg, per in test_cases:
                pid = get_product_id(pkg, per)
                if not pid:
                    all_found = False
                    break
    ...
    console.print("\n" + "=" * 60)
    total = passed + failed
    console.print(f"[bold]总计:[/bold] {passed}通过, {failed}失败, {skipped}跳过 (共{total}项)")
