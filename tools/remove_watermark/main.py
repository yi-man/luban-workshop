...
def detect_watermark(image):
    ...
    x = int(w * 0.8)
    y = int(h * 0.9)
    watermark_width = int(w * 0.2)
    watermark_height = int(h * 0.1)
...
def remove_watermark(input_path, output_path):
    ...
    print(f"水印区域: x={x}, y={y}, w={watermark_width}, h={watermark_height}")
    
    # 直接用背景颜色填充水印区域
    # 取水印区域上方的背景颜色
    if y > 10:
        # 取水印区域上方的背景
        background = image[y-10:y, x:x+watermark_width]
        # 计算平均颜色
        avg_color = np.mean(background, axis=(0, 1))
        print(f"背景平均颜色: {avg_color}")
        # 填充水印区域
        image[y:y+watermark_height, x:x+watermark_width] = avg_color.astype(np.uint8)
