...
用法:
  python3 main.py <分享链接或文本> [输出文件名]
  python3 main.py login <平台>    # 登录并保存 cookie（bilibili/douyin/xiaohongshu）
...
def main():
    if len(sys.argv) < 2:
        print("用法:")
        print("  python3 main.py <分享链接或文本> [输出文件名]  # 下载视频")
        print("  python3 main.py login <平台> [--signal-file F] # 登录保存cookie")
        print("  python3 main.py check-login <平台>             # 检查登录状态")
        print()
    ...
    if sys.argv[1] == 'check-login':
        if len(sys.argv) < 3:
            print("用法: python3 main.py check-login <平台>")
            sys.exit(1)
        platform = sys.argv[2]
    ...
    if sys.argv[1] == 'login':
        if len(sys.argv) < 3:
            print("用法: python3 main.py login <平台> [--signal-file <path>]")
